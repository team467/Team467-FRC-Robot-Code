 ...
 * @author USFIRST
 */
public class Camera2011 implements Runnable
{
    //Team467Camera instance
    private static Camera2011 instance = null;
     ...
     * @return The instance
     */
    public static Camera2011 getInstance()
    {
        if (instance == null)
        {
            instance = new Camera2011();
        }
        return instance;
    }
    ...
    private void detectParticles()
    {
        ...
        try
        {
            thresholdHSL = image.thresholdHSL(90, 255, 0, 255, 125, 255);//(117, 255, 10, 255, 137, 255);
    ...
    private synchronized void setDataFromParticle()
    {
        if ((largestParticle != null) && (largestParticle.boundingRectHeight >= targetMinimumHeight)
                && Math.abs(largestParticle.boundingRectHeight - largestParticle.boundingRectWidth) < (0.25 * largestParticle.boundingRectWidth))
        {
            ...
            camData.targetWidth = largestParticle.boundingRectWidth;
            // TODO - angle should probably be divided by 180.0 to be correct.
