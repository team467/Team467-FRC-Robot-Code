...
public class Team467DriverStation
{
    ...
    //driverstation outputs
    private static final int KICKER_PRIMED_LED = 1;
    private static final int VACUUM_LED = 2;

    //Zone constants
    public static final int ZONE_FRONT = 1;
    public static final int ZONE_MIDDLE = 2;
    public static final int ZONE_BACK = 3;
    private static final double ZONE_FRONT_THRESHOLD = 0.0;//Not actual value
    private static final double ZONE_MIDDLE_THRESHOLD = 0.0;//Not actual value

    //Driverstation Buttons (digital inputs)
    private static final int VACCUM_OVERRIDE = 1;
    private static final int KICKER = 2;
    private static final int HOOK_ARM_RETRACT = 3;
    private static final int HOOK_ARM_EXTEND = 4;
    private static final int HOOK_WINCH_IN = 5;
    private static final int KICKER_OVERRIDE = 6;

    //Analog constatnts (analog inputs)
    private static final int ZONE_SWITCH_INPUT = 1;
    ...
    public boolean VacuumOverride = false;
    public boolean KickerOverride = false;
    public boolean KickerFire = false;
    public boolean HookArmExtend = false;
    public boolean HookArmRetract = false;
    public boolean HookWinchIn = false;
    ...
    public void ReadInputs()
    {
        ...
        VacuumOverride = driverstation.getDigitalIn(VACCUM_OVERRIDE);
        KickerOverride = driverstation.getDigitalIn(KICKER_OVERRIDE);
        KickerFire = !driverstation.getDigitalIn(KICKER);
        HookArmExtend = !driverstation.getDigitalIn(HOOK_ARM_EXTEND);
        HookArmRetract = !driverstation.getDigitalIn(HOOK_ARM_RETRACT);
        HookWinchIn = !driverstation.getDigitalIn(HOOK_WINCH_IN);
        ...
        //Read analog inputs and map to zone
        double zoneSwitch = driverstation.getAnalogIn(ZONE_SWITCH_INPUT);
        if (zoneSwitch < ZONE_FRONT_THRESHOLD)
        {
            Zone = ZONE_FRONT;
        }
        else if (zoneSwitch < ZONE_MIDDLE_THRESHOLD)
        {
            Zone = ZONE_MIDDLE;
        }
        else
    ...
    /**
     * Set status of kicker LED on driverstation
     * @param b - if true, turns on LED. false turns it off
     */
    public void SetKickerLED(boolean b)
    {
        driverstation.setDigitalOut(KICKER_PRIMED_LED, b);
    }
    /**
     * Set status of vacuum LED on driverstation
     * @param b - if true, turns on LED. false turns it off
     */
    public void SetVacuumLED(boolean b)
    {
        driverstation.setDigitalOut(VACUUM_LED, b);
    }
