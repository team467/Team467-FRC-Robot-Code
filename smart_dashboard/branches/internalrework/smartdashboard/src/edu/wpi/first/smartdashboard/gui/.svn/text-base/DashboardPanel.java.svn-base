...
public class DashboardPanel extends JPanel {
    ...
    public DashboardPanel(DashboardFrame frame, ITable table) {
        ...
        edu.wpi.first.smartdashboard.robot.Robot.client.addTableListener(listener, true);
     ...
     * {@link ITable} that have been explicitly declared to be ignored by
     * the user).
    ...
    public void clear() {
        ...
        edu.wpi.first.smartdashboard.robot.Robot.client.removeTableListener(listener);
        edu.wpi.first.smartdashboard.robot.Robot.client.addTableListener(listener, true);
     ...
     * The field will then be added to the list of fields which should remain
     * hidden.
     ...
     * If you would like to remove a {@link Widget} instead of
     * a {@link StaticWidget}, then consider using
     ...
     * Every other element will then be drawn in front of the given one.
     ...
     * If the given point is {@code null}, then it will find a place to put it.
     * If the point is not {@code null}, then the given {@link DisplayElement}
     * should already have set its size.
     ...
     * It will return the forward most element.
    ...
    private class RobotListener implements ITableListener {
		...
		@Override
		public void valueChanged(final ITable source, final String fullKey, final Object value, final boolean isNew) {
                    String tableName = table==edu.wpi.first.smartdashboard.robot.Robot.getLiveWindow()?"LiveWindow":"SmartDashboard";
                    String _key = null;
                    boolean _isTable = true;
                    String[] tmp = fullKey.substring(1).split("/");
                    if(tmp[0].equals(tableName)){
                        if(tmp.length==2){
                            _key = tmp[1];
                            _isTable = false;
                        }
                        else if(tmp.length==3 && tmp[2].equals("~TYPE~"))
                            _key = tmp[1];
                    }
                    final boolean isTable = _isTable;
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						if (!hiddenFields.contains(key)) {
                                                    if(isTable){
							setField(key, null, table.getSubTable(key), null);
                                                    }
                                                    else
