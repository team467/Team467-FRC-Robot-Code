 ...
 * corresponds to that physical subsystem on the robot (e.g. a drive train 
 * would contain speed controllers and encoders).
 * @author Sam Carlberg
 */
public class LWSubsystem extends AbstractTableWidget {
    ...
    /** An implementation of {@link MouseAdapter} that's responsible for dragging widgets. */
    private Mouse mouse;
	...
	public void tableChanged(ITable source, String key, ITable value, boolean isNew) {
        try {
            ...
            if(reader != null) alreadyHasWidget = reader.containsWidgetOfName(this, key);
            if(!alreadyHasWidget) System.out.println("\nSubsystem \""+getFieldName()+"\" does not contain widget \""+key+"\"");
            if(!alreadyHasWidget) {
    ...
    /**
     * Gets all the widgets contained within this
     * subsystem.
     */
    ...
    private class Mouse extends MouseAdapter {
        ...
        public void mouseDragged(MouseEvent e) {
            if(editable) {
                if (selected == null) selected = getWidgetAt(e.getPoint());
                if(selected != null) {
