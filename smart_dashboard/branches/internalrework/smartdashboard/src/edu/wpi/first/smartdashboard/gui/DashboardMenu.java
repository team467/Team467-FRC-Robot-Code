...
public class DashboardMenu extends JMenuBar {
	...
	DashboardMenu(final DashboardFrame frame, final MainPanel mainPanel) {
        ...
        final JCheckBoxMenuItem liveWindowMode = new JCheckBoxMenuItem("Live Window Mode");
        liveWindowMode.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                boolean isInLW = liveWindowMode.isSelected();
                Robot.getLiveWindow().getSubTable("~STATUS~").putBoolean("LW Enabled", isInLW);
                
                frame.setDisplayMode(isInLW ? 
                                    DashboardFrame.DisplayMode.LiveWindow : 
                                    DashboardFrame.DisplayMode.SmartDashboard);
                
                mainPanel.setCurrentPanel(Robot.getLiveWindow().getSubTable("~STATUS~").getBoolean("LW Enabled") ? 
                                          MainPanel.getPanel("LiveWindow") : 
                                          MainPanel.getPanel("SmartDashboard"));
                if(!isInLW) resetLW.doClick();
            }
        });
        liveWindowMode.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F12, 0));
        viewMenu.add(liveWindowMode);
